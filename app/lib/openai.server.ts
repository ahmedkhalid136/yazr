...
const textChat = async (
  ...
  nameFormat?: string,
  model: ModelOptions = ModelOptions.gpt_4o,
  retryNumber: number = 0,
) => {
  let retry = retryNumber;
  try {
    ...
    if (retry < 3) {
      console.log(`Retry attempt ${retry + 1}, waiting 3 seconds...
        ${error}`);
...
const o1Chat = async (
  text: string,
  model: ModelOptions = ModelOptions.o1_mini,
) => {
